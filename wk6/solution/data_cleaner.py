...
for i in range(1, input_sheet.nrows):
    for j in range(0, input_sheet.ncols):
        if input_sheet.cell_value(rowx=i, colx=j) == '':
            output_sheet.write(i, j, random.random())
        elif input_sheet.cell_value(rowx=i, colx=j) > 1:
            output_sheet.write(i, j, random.random())
        elif type(input_sheet.cell_value(rowx=i, colx=j)) == str:
            output_sheet.write(i, j, random.random())
        elif type(input_sheet.cell_value(rowx=i, colx=j)) is None:
            output_sheet.write(i, j, random.random())
        else:
            output_sheet.write(i, j, input_sheet.cell_value(rowx=i, colx=j))
